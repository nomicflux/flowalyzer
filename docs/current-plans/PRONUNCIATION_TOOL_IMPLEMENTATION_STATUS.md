...
- (2025-11-12) Runtime Hang Fix Phase 1 – Diagnostic logging: Added detailed timing logs in `SessionEngine::new` around feature extraction (start, completion, elapsed time, frame counts). Added step-by-step logging in `EngineRunner::build` (reference loading, engine creation, alignment computation) with timing for each phase. Verified error handling in build sends errors to UI via snapshot channel. Commands executed: `cargo fmt`, `cargo clippy --all-targets --all-features`, `cargo test`.
- (2025-11-12) Runtime Hang Fix Phase 2 – Initial snapshot race condition: Added `initializing` field to `SessionSnapshot` (defaults to true). Modified `SessionRuntime::new` to wait for complete snapshot (initializing=false) with 60s timeout, looping through initializing snapshots until complete. Updated `EngineRunner::build` to set initializing=false in initial snapshot. Updated UI to show "Initializing engine... Please wait." when initializing=true. Commands executed: `cargo fmt`, `cargo clippy --all-targets --all-features`, `cargo test`.
- (2025-11-12) Runtime Hang Fix Phase 3 – Async feature extraction: Refactored runtime thread to send initializing snapshot immediately upon spawn, then perform build, then send complete snapshot. Modified `SessionRuntime::new` to wait for complete snapshot (filtering out initializing snapshots). This ensures UI receives feedback immediately and commands are queued until engine is ready. Commands executed: `cargo fmt`, `cargo clippy --all-targets --all-features`, `cargo test`.
