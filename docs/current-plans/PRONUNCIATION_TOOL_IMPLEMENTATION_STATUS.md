...
- Phase 4 alignment pipeline: `src/pronunciation/alignment/` now exposes `PronunciationDictionary` (bundled CMU lexicon), phoneme template synthesis, and a DTW core that yields `AlignmentReport` entries with per-phoneme timing, similarity, articulation variance, and aggregated confidence metrics for downstream scoring/UI.
- Alignment regression fixtures captured under `tests/fixtures/alignment/` with deterministic coverage in `tests/alignment/dictionary.rs`, `tests/alignment/dtw.rs`, and end-to-end verification in `tests/alignment.rs`. Tutorial documentation added at `docs/tutorials/pronunciation_dictionary_alignment.md`.
- Phase 5 metrics engine: `src/pronunciation/metrics/mod.rs` now normalizes timing/articulation/intonation subscores (`TIMING_TOLERANCE_MS = 120`, `ARTICULATION_TOLERANCE = 1.0`) and blends them with alignment confidence (`CONFIDENCE_WEIGHT = 0.3`) to produce `PronunciationScores`; regression coverage added in `tests/metrics.rs`.
- UI architecture established under `src/ui/`: `SessionApp` (eframe 0.24) orchestrates control panel, dual waveform renderer, spectrogram heatmap, and phoneme timeline components. Placeholder visuals derive from alignment metadata pending real capture data; reusable widgets live in `src/ui/components/`, assets staged under `assets/ui/`.
- Manual QA checklist (Phase 5 baseline): run `cargo run --example session_ui`, toggle record/play controls, select phonemes to update detail pane, confirm spectrogram repaint on hover, verify keyboard focus traversal (Tab cycle), close window without panic.
